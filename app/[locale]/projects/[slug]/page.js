...
export async function generateMetadata({ params }) { const { locale, slug } = await params; const p = projects.find(p=>p.slug===slug)?.[locale]; return p ? {title:`${p.title} | Forma Studio`, description:p.intro, alternates:{languages:{en:`/en/projects/${slug}`,tr:`/tr/projects/${slug}`}}} : {}; }
