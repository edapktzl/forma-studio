...
export default async function Home({ params }) {
 ...
 return <>
  ...
  <section className="shell section-pad"><SectionHeading label={c.home.projectLabel} title={c.home.projectTitle} description={c.home.projectText} href={`/${locale}/projects`} link={c.allProjects}/><div className="project-grid featured-projects">{projects.slice(0,2).map(project => <ProjectCard key={project.slug} project={project} locale={locale}/>)}</div></section>
  <section className="testimonial"><div className="shell testimonial-inner"><span className="quote-symbol" aria-hidden="true">“</span><blockquote>{c.home.quote}</blockquote><div><strong>{c.home.quoteName}</strong><span>{c.home.quoteRole}</span></div><div className="quote-index" aria-hidden="true"><i/><span>01 / 01</span></div></div></section>
  <section className="shell section-pad"><SectionHeading label={c.home.journalLabel} title={c.home.journalTitle} description={c.home.journalText} href={`/${locale}/insights`} link={c.allInsights}/><div className="article-grid">{articles.map(article => <ArticleCard key={article.slug} article={article} locale={locale}/>)}</div></section>
