...
export default function MediaPicker({onSelect,onClose}){
 ...
 return <dialog ref={dialog} className="media-dialog" onCancel={onClose} aria-labelledby="media-title"><header><div><p className="eyebrow">{t('IMAGE LIBRARY')}</p><h2 id="media-title">{t('Choose a photograph')}</h2></div><button type="button" onClick={onClose} aria-label={t('Close image library')}>×</button></header><label className="upload-button">{busy?t('Uploading…'):t('+ Upload image')}<input type="file" accept="image/jpeg,image/png,image/webp" disabled={busy} onChange={upload}/></label><p className="field-help">{t('JPEG, PNG or WebP · up to 10 MB · up to 20 megapixels')}</p>{error&&<p className="notice error" role="alert">{error}</p>}{loading?<p role="status">{t('Loading images…')}</p>:items.length?<div className="media-grid">{items.map(item=><button type="button" className="media-tile" key={item.id} onClick={()=>onSelect(item)}><img src={mediaUrl(item.public_url)} alt={item.alt_text||item.file_name}/><span>{item.file_name}</span></button>)}</div>:<div className="empty-state"><h3>{t('Your image library starts here.')}</h3><p>{t('Upload a photograph to use it in your content.')}</p></div>}</dialog>;
}
