...
export default function ArticleList({ locale, limit, editorial = false }) {
 ...
 useEffect(() => {
  ...
  getArticles(locale, page, limit || 12).then((data) => { if (active) setItems(data || []); }).catch(() => { if (active) setError(true); }).finally(() => { if (active) setLoading(false); });
  return () => { active = false; };
