...
function normalizeProjects(items, locale) {
  return items.filter((item) => item?.slug).map((item) => ({
    ...
    video: mediaUrl(item.video_url || item.video) || '',
...
export default function HeroProjectSlider({ locale }) {
  ...
  useEffect(() => {
    ...
    getProjects(locale, { featured: true, page: 1, page_size: 10 })
      .then((data) => {
        ...
        setItems(normalizeProjects(data.items, locale)); setActive(0);
      })
      .catch(() => {})
  ...
  return <div className="hero-image hero-project-slider" role="region" aria-roledescription="carousel" aria-label={locale === 'tr' ? 'Öne çıkan projeler' : 'Featured projects'} onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocus={() => setPaused(true)} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setPaused(false); }}>
    {project.video && !reducedMotion ? <video ref={videoRef} key={project.slug} className="picture hero-video" src={project.video} poster={project.image} autoPlay muted loop playsInline preload="metadata" aria-label={`${project.title} — ${project.concept}`} /> : <Picture key={project.slug} src={project.image} alt={`${project.title} — ${project.concept}`} eager={!loaded && active === 0} />}
    <div className="hero-image-shade" />
    <div className="hero-image-caption" aria-live="polite"><div><span>{project.title}</span><p>{project.concept}{project.concept && project.location ? ' · ' : ''}{project.location}{project.year ? ` · ${project.year}` : ''}</p></div>{items.length > 1 && <div className="hero-slider-controls" aria-label={locale === 'tr' ? 'Proje slayt kontrolleri' : 'Project slide controls'}><button className="hero-slider-control" type="button" onClick={() => goTo(-1)} aria-label={locale === 'tr' ? 'Önceki proje' : 'Previous project'}><ArrowLeft size={19} /></button><button className="hero-slider-control" type="button" onClick={() => goTo(1)} aria-label={locale === 'tr' ? 'Sonraki proje' : 'Next project'}><ArrowRight size={19} /></button></div>}</div>
